...
GameHook<Direct3DCreate9_t> g_direct3DCreate9Hook("Direct3DCreate9");
	...
	IDirect3DDevice9** ppReturnedDeviceInterface)
{
	...
	HRESULT result = g_createDeviceHook.Original()(self, adapter, deviceType, hFocusWindow, behaviorFlags,
		...
		memcmp(&asked, presentParameters, sizeof(asked)) != 0)
	{
		...
		*presentParameters = asked;
		result = g_createDeviceHook.Original()(self, adapter, deviceType, hFocusWindow, behaviorFlags,
			presentParameters, ppReturnedDeviceInterface);
	}
...
bool HookCreateDeviceFrom(IDirect3D9* d3d9)
{
	...
	if (!g_createDeviceHook.Install(vtable[kCreateDeviceIndex], &HookedCreateDevice))
		return false;
...
Direct3DCreate9_t Creator()
{
	...
	return chosen != nullptr ? chosen : g_direct3DCreate9Hook.Original();
}
...
{
	if (g_createDeviceHooked || !g_direct3DCreate9Hook.IsLive())
		return;
	...
	IDirect3D9* const probe = g_direct3DCreate9Hook.Original()(D3D_SDK_VERSION);
...
bool D3D9Wrapper::InstallHooks()
{
	...
	if (!g_direct3DCreate9Hook.InstallApi("d3d9.dll", "Direct3DCreate9", &HookedDirect3DCreate9))
		return false;
