...
GameHook<SetVolumeFn> g_setVolumeHook("BgmSetVolume");
GameHook<StreamVolumeFn> g_streamVolumeHook("StreamSetVolume");
...
{
	g_inRoutine = true;
	g_setVolumeHook.Original()(base, track);
	g_inRoutine = false;
}
...
int __fastcall HookedStreamVolume(void* stream, void* unused, int scalar)
{
	...
	if (g_inRoutine || scalar <= 0 || !IsBgmStream(stream) || !CurrentWanted(wanted, slot, id))
		return g_streamVolumeHook.Original()(stream, unused, scalar);
	...
	return g_streamVolumeHook.Original()(stream, unused, scaled);
}

void InstallStreamHook()
{
	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnStreamSetVolume));

	if (!IsAddressInGameModule(reinterpret_cast<uintptr_t>(target)))
		return;

	if (g_streamVolumeHook.Install(target, &HookedStreamVolume))
		return;
...
void BgmVolume::ApplyNow()
{
	...
	if (!g_setVolumeHook.IsLive() || !CurrentWanted(wanted))
		return;
...
bool BgmVolume::Install()
{
	...
	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnBgmSetVolume));
	...
	if (g_setVolumeHook.Install(target, &HookedSetVolume))
		return true;
