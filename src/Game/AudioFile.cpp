...
bool ConvertWav(const std::string& source, OggWriter& writer, const std::string& target,
	char* status, int statusSize)
{
	const std::vector<uint8_t> file = ReadWhole(source);
	...
	if (!writer.Begin(target, wav.channels, wav.rate, status, statusSize))
		return false;
	...
	for (size_t done = 0; done < frames;)
	{
		...
		writer.Write(block.data(), static_cast<int>(take));
		done += take;
	}

	LOG("AudioFile: read %s, %d Hz, %d channel(s), %d bit%s", source.c_str(), wav.rate,
		wav.channels, wav.bits, wav.isFloat ? " float" : "");
...
public:
	...
	void Push(const short* samples, int frames, OggWriter& writer)
	{
		...
		writer.Write(m_held.data(), ready);
...
bool ConvertMp3(const std::string& source, OggWriter& writer, const std::string& target,
	char* status, int statusSize)
{
	const std::vector<uint8_t> file = ReadWhole(source);
	...
	while (offset < static_cast<int>(size))
	{
		...
		if (feed == nullptr)
		{
			...
			if (!writer.Begin(target, channels, rate, status, statusSize))
				return false;

			feed.reset(new GaplessFeed(gapless, channels));

			LOG("AudioFile: read %s, %d Hz, %d channel(s), %d kbps", source.c_str(), rate, channels,
				info.bitrate_kbps);
		}
...
bool Decode(AudioFile::Format format, const std::string& source, OggWriter& writer,
	const std::string& target, char* status, int statusSize)
{
	if (format == AudioFile::Format_Mp3)
		return ConvertMp3(source, writer, target, status, statusSize);

	return ConvertWav(source, writer, target, status, statusSize);
}
...
AudioFile::Format AudioFile::Identify(const std::string& path)
{
	...
	if (!ReadHeader(path, header, kHeaderBytes, read) || read < 16)
		return Format_Unknown;

	if (IsWav(header, read))
		return Format_Wav;

	if (Tag(header, read, 0, "OggS"))
		return Contains(header, read, "vorbis") ? Format_OggVorbis : Format_OggOther;

	if (IsMp3(header, read))
		return Format_Mp3;
	...
	int statusSize)
{
	...
	if (!Decode(format, source, writer, target, status, statusSize))
	{
		writer.Abort();
