...
GameHook<MinDamageFn> g_minDamageHook("MinGuaranteedDamage");
...
int __fastcall HookedMinDamage(void* self, void* unused, int damage)
{
	if (!g_active[0] || self == nullptr)
		return g_minDamageHook.Original()(self, unused, damage);
...
void Recompute()
{
	...
	{
		const bool wanted = g_minDamageHook.IsLive() && g_version > 0 && g_version < kRules[i].since;
...
{
	void* const target =
		reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnMinGuaranteedDamage));
	...
	if (!g_minDamageHook.Install(target, &HookedMinDamage))
	{
		Summarise();
		return false;
	}
