...
GameHook<GetPP_t> g_getPPHook("GetPP");
GameHook<PlayerDataDtor_t> g_playerDataDtorHook("PLAYER_DATA::~PLAYER_DATA");
...
{
	Forget(thisPtr);
	g_playerDataDtorHook.Original()(thisPtr, unused);
}
...
int __cdecl HookedGetPP(int index)
{
	...
	return g_getPPHook.Original()(index);
}
...
bool CharaTracker::Install()
{
	...
	void* target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnGetPP));

	if (!g_getPPHook.Install(target, &HookedGetPP))
		return false;

	void* dtor = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnPlayerDataDestructor));
	g_playerDataDtorHook.Install(dtor, &HookedPlayerDataDtor);
