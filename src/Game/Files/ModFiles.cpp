...
GameHook<CreateFileA_t> g_createFileAHook("CreateFileA");
GameHook<GetFileAttributesA_t> g_getFileAttributesAHook("GetFileAttributesA");
GameHook<CreateFileW_t> g_createFileWHook("CreateFileW");
GameHook<GetFileAttributesW_t> g_getFileAttributesWHook("GetFileAttributesW");
	...
	LPSECURITY_ATTRIBUTES security, DWORD creation, DWORD flags, HANDLE templateFile)
{
	...
	{
		return g_createFileAHook.Original()(replacement.c_str(), access, share, security, creation, flags,
			templateFile);
	}

	return g_createFileAHook.Original()(fileName, access, share, security, creation, flags, templateFile);
}
...
DWORD WINAPI HookedGetFileAttributesA(LPCSTR fileName)
{
	...
	if (Redirect(fileName, replacement))
		return g_getFileAttributesAHook.Original()(replacement.c_str());

	return g_getFileAttributesAHook.Original()(fileName);
}
	...
	LPSECURITY_ATTRIBUTES security, DWORD creation, DWORD flags, HANDLE templateFile)
{
	...
	{
		return g_createFileWHook.Original()(Widen(replacement).c_str(), access, share, security, creation, flags,
			templateFile);
	}

	return g_createFileWHook.Original()(fileName, access, share, security, creation, flags, templateFile);
}
...
DWORD WINAPI HookedGetFileAttributesW(LPCWSTR fileName)
{
	...
	if (Narrow(fileName, asked) && Redirect(asked.c_str(), replacement))
		return g_getFileAttributesWHook.Original()(Widen(replacement).c_str());

	return g_getFileAttributesWHook.Original()(fileName);
}
...
bool ModFiles::Initialize()
{
	...
	const bool create = g_createFileAHook.InstallApi("kernel32.dll", "CreateFileA", &HookedCreateFileA);

	const bool attributes = g_getFileAttributesAHook.InstallApi("kernel32.dll", "GetFileAttributesA", &HookedGetFileAttributesA);

	const bool createWide = g_createFileWHook.InstallApi("kernel32.dll", "CreateFileW", &HookedCreateFileW);

	const bool attributesWide = g_getFileAttributesWHook.InstallApi("kernel32.dll", "GetFileAttributesW", &HookedGetFileAttributesW);
