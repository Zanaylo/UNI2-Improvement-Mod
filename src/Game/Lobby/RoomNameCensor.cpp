...
GameHook<SetRoomRowsFn> g_setRoomRowsHook("SetRoomRows");
...
{
	CoverAll(rows, count);
	g_setRoomRowsHook.Original()(list, rows, count);
}
...
bool RoomNameCensor::Install()
{
	...
	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnSetRoomRows));

	if (!IsAddressInGameModule(reinterpret_cast<uintptr_t>(target)) ||
		!g_setRoomRowsHook.Install(target, &HookedSetRoomRows))
	{
		Summarise();
...
{
	return g_setRoomRowsHook.IsLive();
}
