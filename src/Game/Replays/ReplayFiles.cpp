...
#include "Game/Replays/ReplayArchive.h"
...
GameHook<SampleKeyboardFn> g_sampleKeyboardHook("SampleKeyboardForReplay");
...
{
	return g_sampleKeyboardHook.IsLive() && !GameState::IsInMatch() &&
		CodeSignatures::Address(GameOffsets::kFnPlayReplayRecord) != 0;
}
...
void ReplayFiles::OnGameFrame()
{
	...
	const uintptr_t play = CodeSignatures::Address(GameOffsets::kFnPlayReplayRecord);
...
{
	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnSampleKeyboard));

	if (target == nullptr || !g_sampleKeyboardHook.Install(target, &HookedSampleKeyboard))
	{
...
{
	const uintptr_t pop = CodeSignatures::Address(GameOffsets::kFnPopInputBlock);
