...
int __fastcall HookedSetup(void* self, void* unused)
{
	...
	const int result = oSetup(self, unused);
...
bool StageCards::Initialize()
{
	...
	if (!HookManager::CreateAndEnableHook(reinterpret_cast<void*>(address), &HookedSetup,
		reinterpret_cast<void**>(&oSetup), "StageSelectSetup"))
	{
		strncpy_s(g_status, "the stage picker's setup could not be hooked", _TRUNCATE);
		return false;
	}

	if (!HookManager::CreateApiHook("d3dx9_42.dll", "D3DXCreateTextureFromFileInMemoryEx",
		&HookedCreateTexture, reinterpret_cast<void**>(&oCreateTexture)))
	{
