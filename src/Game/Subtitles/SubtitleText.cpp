...
GameHook<ExStringTextFn> g_exStringTextHook("ExStringText");
...
const char* __cdecl HookedExStringText(const char* key)
{
	...
	if (!SubtitleWatch::IsEnabled() || !Named(key, chara, index))
		return g_exStringTextHook.Original()(key);
...
{
	if (g_exStringTextHook.IsLive())
		return true;

	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnExStringText));
	...
	if (!g_exStringTextHook.Install(target, &HookedExStringText))
	{
...
{
	return g_exStringTextHook.IsLive();
}
...
bool SubtitleText::Draw(int slot, int chara, int index, int frames)
{
	...
	const uintptr_t target = CodeSignatures::Address(GameOffsets::kFnCharaSetMessage);
