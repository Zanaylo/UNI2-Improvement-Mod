...
GameHook<SePlayFn> g_sePlayHook("SeManagerPlay");
GameHook<SeTableLoadFn> g_seTableLoadHook("BattlePlayerSeLoad");
...
void Summarise()
{
	...
	strncpy_s(g_status, g_seTableLoadHook.IsLive() ? "subtitles are on"
...
void __fastcall HookedSeTableLoad(const char* folder, int slot)
{
	...
	g_seTableLoadHook.Original()(folder, slot);
}
...
{
	const int played = g_sePlayHook.Original()(self, unused, group, index, params);
...
void InstallTableLoad()
{
	...
	if (g_seTableLoadHook.Install(target, &HookedSeTableLoad))
		return;
...
bool SubtitleWatch::Install()
{
	...
	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnSePlay));
	...
	if (!g_sePlayHook.Install(target, &HookedSePlay))
	{
		Summarise();
		LOG("SubtitleWatch: %s", g_status);
		return false;
