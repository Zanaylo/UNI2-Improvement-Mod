...
GameHook<ChooseSessionFn> g_chooseSessionHook("ChooseRoomSession");
...
bool CallStartSpectator(void* holder)
{
	__try
	{
		reinterpret_cast<StartSpectatorFn>(CodeSignatures::Address(GameOffsets::kFnStartSpectatorSession))(holder);
	}
...
void __fastcall HookedChooseSession(void* holder, void* edx, int first, int second)
{
	...
	{
		g_chooseSessionHook.Original()(holder, edx, first, second);
		return;
	}
...
bool EndSession()
{
	...
	__try
	{
		reinterpret_cast<TeardownFn>(CodeSignatures::Address(GameOffsets::kFnSessionTeardown))();
	}
	__except (EXCEPTION_EXECUTE_HANDLER)
	{
		return false;
...
bool GoToMenu()
{
	...
	__try
	{
		reinterpret_cast<DispatchFn>(CodeSignatures::Address(GameOffsets::kFnMatchDispatch))(command);
	}
	__except (EXCEPTION_EXECUTE_HANDLER)
	{
		return false;
...
{
	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnChooseRoomSession));

	if (g_chooseSessionHook.IsLive())
	...
	if (IsAddressInGameModule(reinterpret_cast<uintptr_t>(target)) &&
		g_chooseSessionHook.Install(target, &HookedChooseSession))
	{
		return true;
	}
