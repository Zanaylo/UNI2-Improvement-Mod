...
HitboxOverlay* Instance()
{
	const WindowContainer* const container = WindowManager::GetInstance().GetContainer();

	return container == nullptr ? nullptr
		: container->GetWindow<HitboxOverlay>(WindowType_HitboxOverlay);
}

}

bool HitboxOverlay::IsShown()
{
	const HitboxOverlay* const overlay = Instance();

	return overlay != nullptr && overlay->IsOpen();
}

void HitboxOverlay::SetShown(bool shown)
{
	HitboxOverlay* const overlay = Instance();
...
void HitboxOverlay::Draw()
{
	...
	if (!GameState::IsInMatch() || OnlineState::IsOnline() || !Camera::IsAvailable() || TrainingMenu::IsHeld())
		return;
