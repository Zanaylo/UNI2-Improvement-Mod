...
GameHook<Promote_t> g_promoteHook("DummyRecorder::Promote");
...
void __fastcall HookedPromote(void* thisPtr, void* unused)
{
	...
	{
		g_promoteHook.Original()(thisPtr, unused);
		return;
	}
...
bool DummyRecorder::Install()
{
	...
	void* target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnDummyPromote));
	if (target == nullptr)
		return false;

	if (!g_promoteHook.Create(target, &HookedPromote))
		return false;
	...
	if (g_hasPending)
	{
		...
		if (g_leadInRemaining <= 0)
		{
			...
			if (g_restartAtEnd)
			{
				...
				g_replaying = true;
				g_promoteHook.Original()(g_pendingThis, g_pendingUnused);
				g_replaying = false;
			}
...
constexpr size_t kRuntimeDwords =
	(GameOffsets::kRecorderWatchRuntimeEnd - GameOffsets::kRecorderWatchRuntimeStart) / 4;
constexpr size_t kSettingsDwords =
	(GameOffsets::kRecorderWatchSettingsEnd - GameOffsets::kRecorderWatchSettingsStart) / 4;
constexpr size_t kCharaDwords =
	(GameOffsets::kRecorderWatchCharaEnd - GameOffsets::kRecorderWatchCharaStart) / 4;
...
uintptr_t WatchAddress(size_t index)
{
	if (index < kRuntimeDwords)
		return GameOffsets::kRecorderWatchRuntimeStart + index * 4;

	if (index < kRuntimeDwords + kSettingsDwords)
		return GameOffsets::kRecorderWatchSettingsStart + (index - kRuntimeDwords) * 4;

	return GameOffsets::kRecorderWatchCharaStart + (index - kRuntimeDwords - kSettingsDwords) * 4;
}
