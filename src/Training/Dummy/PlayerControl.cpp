...
GameHook<FetchPadFn> g_fetchPadHook("FetchPad");
...
RequestSlot<int, 0> g_releaseRequest;
RequestSlot<int, 0> g_calibrateRequest;
...
{
	g_fetchPadHook.Original()(out, player);
...
{
	void* const target = reinterpret_cast<void*>(CodeSignatures::Address(GameOffsets::kFnFetchPad));

	if (!g_fetchPadHook.Install(target, &HookedFetchPad))
	{
...
void PlayerControl::Calibrate()
{
	...
	g_calibrateRequest.Post(1);
	sprintf_s(g_status, "calibrating button 1 of %d", kCalibrateByteCount);
}

bool PlayerControl::IsCalibrating()
{
	return g_calibrating || g_calibrateRequest.IsPending();
}
...
void PlayerControl::OnFrameUpdate()
{
	...
	if (g_releaseRequest.Take(request))
		ReleaseNow();

	if (!GameState::AllowsTrainingTools())
	{
		g_calibrateRequest.Clear();
		ReleaseNow();
		return;
	}

	if (g_calibrateRequest.Take(request))
		StartCalibration();
